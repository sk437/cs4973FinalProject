...
if __name__ == "__main__":
    ...
    parser.add_argument("evaluation_tasks", help="File which contains tasks for evaluation, please use a file with tasks both comparing models are trained on - generate this using AssignTasks.py")
    parser.add_argument("model_directory_0", help="Name of the folder to load the model from - include the checkpoint folder in this")
    parser.add_argument("model_directory_1", help="Name of the folder to load the worse model/model for comparison from - include the checkpoint folder in this")
    parser.add_argument("model_0", help="Which model type to evaluate - please use t5 or bart")
    parser.add_argument("model_1", help="Which model type to compare to - please use t5 or bart")
    ...
    if (config['model_0'] == 't5'):
        model = T5ForConditionalGeneration.from_pretrained(config['model_directory_0'])
        tokenizer = T5Tokenizer.from_pretrained('google/flan-t5-small')
        max_length = 512
    elif (config['model_0'] == 'bart'):
        model = BartForConditionalGeneration.from_pretrained(config['model_directory_0'])
        tokenizer = BartTokenizer.from_pretrained('facebook/bart-base')
        max_length = 1024

    if (config['model_1'] == 't5'):
        model_compare = T5ForConditionalGeneration.from_pretrained(config['model_directory_1'])
        tokenizer_compare = T5Tokenizer.from_pretrained('google/flan-t5-small')
        max_length_1 = 512
    elif (config['model_1'] == 'bart'):
        model_compare = BartForConditionalGeneration.from_pretrained(config['model_directory_1'])
        tokenizer_compare = BartTokenizer.from_pretrained('facebook/bart-base')
        max_length_1 = 1024
        ...
        for task in evaluationPrompts[category]:
            with open('./app_static_tasks_sample/' + task + '.json') as json_file:
                ...
                for i in tqdm(range(min(1000, len(subtasks)))):
                    ...
                    inputs_compare = tokenizer_compare(string_encoding, return_tensors="pt", truncation=True)
                    inputs.to(device)
                    inputs_compare.to(device)
                    outputs = model.generate(**inputs, max_length=128)
                    outputs_compare = model_compare.generate(**inputs_compare, max_length=128)
                    rgeScores = scorer(instance['output'], tokenizer.batch_decode(outputs)[0])
                    rgeScores_compare = scorer(instance['output'], tokenizer_compare.batch_decode(outputs_compare)[0])
                    scoresForCategory.append(rgeScores['rougeL_fmeasure'].item())
                    difference = rgeScores['rougeL_fmeasure'].item() - rgeScores_compare['rougeL_fmeasure'].item()
                    if (difference < 0.1): continue
                    if (difference not in questionsByDifference): questionsByDifference[difference] = []
                    questionsByDifference[difference].append((string_encoding, tokenizer.batch_decode(outputs)[0], tokenizer_compare.batch_decode(outputs_compare)[0]))
        scoreForCategory = sum(scoresForCategory) / len(scoresForCategory)
        scores[category] = scoreForCategory

    keys = list(questionsByDifference.keys())
    keys.sort(reverse=True)
    for key in keys[:10]:
        print(key)
        for comparison in questionsByDifference[key]:
            print(comparison[0])
            print("\nGood Answer: " + comparison[1])
            print("\nBad Answer: " + comparison[2])
